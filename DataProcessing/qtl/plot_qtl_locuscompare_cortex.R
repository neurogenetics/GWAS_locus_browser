...
plots_to_plot <- fread("all_qtl_info_cortex.csv")
plots_to_plot <- plots_to_plot[which(plots_to_plot$feature=="cortex"),]
...
for(rownum in  1:nrow(plots_to_plot))
{
  ...
  if(row$can_plot == TRUE)
  {
    ...
    if(is.na(transcript))
    {
      ...
      gwas_fn = paste0('qtl_temp/', gene, '_gwas.tsv')
      eqtl_fn = paste0('qtl_temp/', gene, '_eqtl.tsv')
    }
    else
    {
      #get file path strings
    ...
    if(feature =='blood' || feature=='brain' || feature == "cortex")
    {
    ...
    if(feature=='cortex')
    {
      feature_axis_label <- paste0('Cortex eQTL, ',gene)
    }
