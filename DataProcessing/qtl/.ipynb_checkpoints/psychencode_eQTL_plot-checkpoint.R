...
# - **Quick Description:** code to make locus compare plots from psychENCODE eQTL data
...
plots_to_plot <- fread("$PATH1/eqtl_plots_to_plot.csv")
...
evidence_merged$'eQTL' <- 0

evidence_merged$'eQTL' <- as.numeric(evidence_merged$'eQTL')
...
evidence_merged$'eQTL corrval' <- NA

evidence_merged$'eQTL corrval' <- as.numeric(evidence_merged$'eQTL corrval')
...
for(rownum in  1:nrow(plots_to_plot))
{
  ...
  if(nrow(df)>=30)
  {
    ...
    if(feature == "eqtl")
    {
      evidence_merged[which(evidence_merged$Gene == gene & evidence_merged$Locusnumber == locus & evidence_merged$SNP == snp),]$'eQTL corrval' <- correlation
  ...
  if(generatePlots)
  {
    ...
    if(feature == "eqtl")
    {
      ...
      plot_file <- paste0('$PATH1/p_eQTL/', gene, '_', snp, '.png')
      k = try(locuscompare(in_fn1 = qtl_fn, in_fn2 = gwas_fn, snp=row$forced_lead_variant, 
                           title = paste0('Psychencode eQTL, ',gene), title2 = 'GWAS'),
              silent = T)
      ...
      if(inherits(k, 'try-error')){
        ...
        evidence_merged[which(evidence_merged$Gene == gene & evidence_merged$Locusnumber == locus & evidence_merged$SNP == snp),]$'eQTL' <- 0
      ...
      else
      {
        ...
        evidence_merged[which(evidence_merged$Gene == gene & evidence_merged$Locusnumber == locus & evidence_merged$SNP == snp),]$'eQTL' <- 1
  ...
  hasPlots <- (evidence_merged[which(evidence_merged$Gene == gene & evidence_merged$Locusnumber == locus & evidence_merged$SNP == snp),]$'eQTL'==1)
  eQTLCorrel <- evidence_merged[which(evidence_merged$Gene == gene & evidence_merged$Locusnumber == locus & evidence_merged$SNP == snp),]$'eQTL corrval'

  if(!is.na(eQTLCorrel))
  {
    evidence_merged[which(evidence_merged$Gene == gene & evidence_merged$Locusnumber == locus & evidence_merged$SNP == snp),]$'QTL-correl' <- ifelse((eQTLCorrel < -.3 || eQTLCorrel > .3), 1, 0)
  }
  if(is.na(eQTLCorrel) || !(hasPlots))
  {
...
write.csv(plots_to_plot, file = "psychencode_eqtl_plot_info.csv", row.names = FALSE)
